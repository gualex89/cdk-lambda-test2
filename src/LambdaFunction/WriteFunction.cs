    ...
    public async Task<object> FunctionHandler(object input, ILambdaContext context)
    {
        ...
        await using (var conn = new NpgsqlConnection(connectionString))
        {
            ...
            foreach (var row in items)
            {
                ...
                cmd.Parameters.AddWithValue("c5", row["prioridad"].ToString());
                cmd.Parameters.AddWithValue("c6", row["fecha_creacion"].ToString());
                cmd.Parameters.AddWithValue("c7", row["fecha_materializacion"].ToString());
                cmd.Parameters.AddWithValue("c8", row["monto_solicitado"].ToString());
